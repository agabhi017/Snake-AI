...
def current_state(prev_action, snake, snack, rows, k):
    ...
    dis_x = snake_head_pos[0] - snack_pos[0]
    dis_y = snake_head_pos[1] - snack_pos[1]
...
dis_xs = np.arange(-9, 10)
dis_ys = np.arange(-9, 10)
gaps_up = [0, 1]
gaps_left = [0, 1]
gaps_right = [0, 1]
snake_len_v = [0, 1]
...
def q_matrix_update(current_state, current_action, reward, next_state):
    ...
    delta = alpha * (reward + gamma * max_reward_next_state - q_mat[state_counter, current_state])
    q_mat[state_counter, current_action] += delta
    return q_mat
...
def action(current_state):
    ...
    if explore :
        ...
        return np.argmax(q_mat[current_state])
...
def main():
    ...
    while steps <= 1000 and episodes <= 100:
        pygame.time.delay(1)
        clock.tick(10000)
        act_0, d_x_0, d_y_0, gap_up_0, gap_left_0, gap_right_0, snake_len_f_0 = current_state(last_move, s, snack, rows, 2)
        lst_0 = [act_0, d_x_0, d_y_0, gap_up_0, gap_left_0, gap_right_0, snake_len_f_0]
        s.move()
        act, d_x, d_y, gap_up, gap_left, gap_right, snake_len_f = current_state(s.last_move, s, snack, rows, 2)
